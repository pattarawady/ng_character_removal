...
const DP_FORBIDDEN_WORDS_RAW: string[] = [
  ...
  // Note: Some words like "JRおすすめ", "JR東海おすすめ" etc. are repeated in the original list.
  // Set will handle deduplication. Case sensitivity is maintained from the original list.
];

// Process list: unique, sort by length descending for safer regex replacement.
...
const DPForbiddenCharHighlighter: React.FC = () => {
  ...
  const handleHighlightText = useCallback(() => {
    ...
    try {
      ...
      setHighlightedText(inputText); // Fallback to show original text
    }
  ...
  const WordDisplay: React.FC<{ title: string; words: string[]; className?: string }> = ({ title, words, className }) => (
    <div className={`mb-3 ${className}`}>
      ...
      <div className="p-3 bg-slate-700 rounded text-xs text-slate-300 max-h-40 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-600 scrollbar-track-slate-700">
        {words.map((word, index) => (
          <span key={index} className="inline-block mr-2 mb-1 p-1 bg-slate-600 rounded hover:bg-slate-500">
            {word}
          </span>
        ))}
      </div>
  ...
  return (
    <div className="w-full p-4 md:p-6 my-0 bg-slate-800 shadow-xl rounded-b-xl rounded-tr-xl ring-1 ring-slate-700">
      ...
      <div className="grid md:grid-cols-2 gap-x-6 gap-y-2">
        ...
        {/* Forbidden Words Display Column */}
        <div className="space-y-5">
           <div className="p-4 bg-slate-850 rounded-lg h-full flex flex-col">
              <h3 className="text-md font-semibold text-slate-200 mb-3">DP規定 禁止語句リスト</h3>
      ...
      <div className="flex flex-col sm:flex-row gap-4 mt-8 mb-6">
        <Button
          ...
          label="禁止語句をハイライト"
          variant="primary"
          // Consider a highlight icon if available, e.g., EyeIcon or similar
          ...
          disabled={!canProcess || !!error && error !== "処理対象のテキストを入力してください。"} // Allow processing if only input text error
          className="flex-1"
        />
        <Button
